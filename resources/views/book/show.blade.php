        ...
        <div class="w-full bg-white rounded-lg p-3">
            ...
            <div class="grid grid-cols-4 gap-6 mt-3 relative">
                <div class="flex flex-col items-center sticky top-0">
                    ...
                    <button class="w-full mt-3 transition-all duration-500 enabled:bg-gradient-to-br enabled:from-red-400 enabled:to-red-600 rounded-lg text-white font-medium p-4 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 text-center hover:bg-blue-600 text-sm shadow-lg hover:shadow-xl shadow-blue-200 hover:shadow-blue-200 focus:shadow-none disabled:shadow-none disabled:bg-slate-700 disabled:cursor-not-allowed">
                    @if ($book->file)
                            <a href="{{ route('books.read', $book->id) }}" method="POST" target="_BLANK" ></a>
                                @endif
                    Download PDF
                </button>
                </div>
                            ...
                            @if ($book->histories)
                                @if ($book->histories->isNotEmpty())
                                    @foreach ($book->histories->unique('user_id') as $history)
                                        <img src="{{ asset('storage/' . $history->user->image) }}" alt="{{ $history->user->name }}" class="w-10 h-10 overflow-hidden rounded-full object-cover border border-white relative -ml-3">
                                    @endforeach
                                @endif
                                <span class="text-slate-700 @if ($book->histories->isNotEmpty()) ml-2 @else mr-2 @endif">{{ $book->histories->count() }} people have read</span>
                            @endif
                        </div>
